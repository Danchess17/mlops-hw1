...
echo "=========================================="
echo "Сборка Docker образа и копирование wheel"
echo "=========================================="

echo -e "\n[1/4] Сборка Docker образа..."
docker build -t cholesky-module .

echo -e "\n[2/4] Проверка wheel файла в контейнере..."
WHEEL_FILE=$(docker run --rm cholesky-module sh -c "ls dist/*.whl" | head -1 | xargs -n1 basename)
...
mkdir -p dist
CONTAINER_ID=$(docker create cholesky-module)
